...
| `Diarization failed ... hf_hub_download() got an unexpected keyword argument 'use_auth_token'` (pipeline-ul continuă pe fallback, dar fără voci) | wrapper-ul de diarizare din whisperx e incompatibil cu huggingface_hub nou (argument redenumit) — codul cheamă acum pyannote direct, cu detectarea automată a numelui de argument; `git pull` + `build` la worker |
